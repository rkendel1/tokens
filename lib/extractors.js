...
export async function extractBranding(
  ...
  try {
    ...
    console.log(contacts.emails?.length > 0 || contacts.phones?.length > 0 || contacts.addresses?.length > 0 ? chalk.hex('#50FA7B')(`  ✓ Contacts: ${contacts.emails?.length || 0} emails, ${contacts.phones?.length || 0} phones, ${contacts.addresses?.length || 0} addresses`) : chalk.hex('#FFB86C')(`  ⚠ Contacts: 0 found`));
    console.log();
