...
export async function extractBranding(
  ...
  try {
    ...
    // Always log contacts extraction results - even if empty
    if (!contacts) {
      console.log(chalk.hex('#FF5555')(`  ✗ Contacts: extraction returned null/undefined`));
    } else if (contacts.emails?.length > 0 || contacts.phones?.length > 0 || contacts.addresses?.length > 0 || contacts.hours?.length > 0 || contacts.names?.length > 0) {
      console.log(chalk.hex('#50FA7B')(`  ✓ Contacts: ${contacts.emails?.length || 0} emails, ${contacts.phones?.length || 0} phones, ${contacts.addresses?.length || 0} addresses, ${contacts.hours?.length || 0} hours, ${contacts.names?.length || 0} names`));
    } else {
      console.log(chalk.hex('#FFB86C')(`  ⚠ Contacts: 0 found`));
    }
    console.log();
...
/**
 * Extract contact information: emails, phone numbers, and addresses
 */
async function extractContacts(page) {
  return await page.evaluate(() => {
    try {
      ...
      let pageText = document.body ? document.body.textContent || '' : ''; // Full page text for broader search
      const mailtoLinks = [];
      const telLinks = [];
      const addressBlocks = [];
      const hoursBlocks = [];
      ...
      contactElements.forEach(el => {
        if (el.tagName === 'A' && el.href.startsWith('mailto:')) {
          ...
          // Get text, avoiding scripts/styles
          if (el.tagName !== 'SCRIPT' && el.tagName !== 'STYLE' && el.textContent) {
            const text = el.textContent.trim();
            if (text) {
              pageText += ' ' + text;
              // For potential address blocks (e.g., in <div class="address">)
              if (el.matches('address, [class*="address"], .contact-info') && text.length > 20) {
                addressBlocks.push(text);
              }
              // For potential hours blocks (e.g., in <div class="hours">)
              if (el.matches('[class*="hour"], .hours, .opening-hours, .business-hours, time') && text.length > 5) {
                hoursBlocks.push(text);
              }
            }
          }
        }
      ...
      // Enhanced phone regex: handles (123) 456-7890, +1 123 456 7890, 123-456-7890, 123.456.7890, ext.123
      const phoneRegex = /(?:(?:\+?(\d{1,3}))?[-.\s(]?(\d{1,4})[)\s-]?(?:[-.\s]?(\d{1,4})[-.\s]?(\d{1,4})[-.\s]?(\d{1,9}))|ext\.?\s?(\d{1,5}))(?:\s*\*?\s*\d+)?/gi;
      // Enhanced address regex: more flexible for streets, cities, states, ZIP (US-focused but broader)
      // Pattern: "123 Main St, City, ST 12345" or "123 Main St #456, City, ST 12345"
      // Using non-greedy matching (+?) which is safe here because the pattern has sufficient
      // structural anchors (street suffix, comma, state, ZIP) that force appropriate expansion.
      // Non-greedy prevents potential over-matching in edge cases while still capturing complete addresses.
      ...
      // Hours regex: matches patterns like "9:00 AM - 5:00 PM", "9am-5pm", "Monday-Friday: 9-5", "24/7", "Open: 9:00-17:00"
      // Components:
      // 1. Optional day range: Monday-Friday, Mon-Fri, Weekday, Weekend
      // 2. Optional colon separator
      // 3. Time range: HH:MM AM/PM - HH:MM AM/PM (various formats)
      // 4. Special cases: 24/7, Open/Hours prefixes
      const hoursRegex = /(?:(?:Mon(?:day)?|Tue(?:sday)?|Wed(?:nesday)?|Thu(?:rsday)?|Fri(?:day)?|Sat(?:urday)?|Sun(?:day)?|Weekday|Weekend)(?:\s*-\s*(?:Mon(?:day)?|Tue(?:sday)?|Wed(?:nesday)?|Thu(?:rsday)?|Fri(?:day)?|Sat(?:urday)?|Sun(?:day)?))?)?\s*:?\s*(?:\d{1,2}(?::\d{2})?\s*(?:AM|PM|am|pm)?)\s*(?:-|to|–|—)\s*(?:\d{1,2}(?::\d{2})?\s*(?:AM|PM|am|pm)?)|24\s*[\/\-]\s*7|Open\s*(?:\d{1,2}(?::\d{2})?\s*(?:AM|PM|am|pm)?)\s*(?:-|to|–|—)\s*(?:\d{1,2}(?::\d{2})?\s*(?:AM|PM|am|pm)?)|Hours\s*:?\s*(?:\d{1,2}(?::\d{2})?\s*(?:AM|PM|am|pm)?)\s*(?:-|to|–|—)\s*(?:\d{1,2}(?::\d{2})?\s*(?:AM|PM|am|pm)?)/gi;
      ...
      let addressesFromText = [...(pageText.match(addressRegex) || [])].filter(addr => addr.length > 10 && addr.match(/\d{5}/)); // Require ZIP-like
      const hoursFromText = [...(pageText.match(hoursRegex) || [])].filter(hour => hour.length > 5 && hour.length < 100);

      // Add address blocks if they match patterns
      addressBlocks.forEach(block => {
        const matches = block.match(addressRegex);
        if (matches) addressesFromText.push(...matches);
      });

      // Add hours blocks
      const allHoursBlocks = [];
      hoursBlocks.forEach(block => {
        const matches = block.match(hoursRegex);
        if (matches) {
          allHoursBlocks.push(...matches);
        } else if (block.length > 5 && block.length < 100) {
          // Include blocks that look like hours even if they don't match the regex
          // (e.g., "Closed on weekends")
          const lowerBlock = block.toLowerCase(); // Cache lowercase version
          if (lowerBlock.includes('hour') || 
              lowerBlock.includes('open') || 
              lowerBlock.includes('close') ||
              lowerBlock.includes('mon') ||
              lowerBlock.includes('tue') ||
              lowerBlock.includes('wed') ||
              lowerBlock.includes('thu') ||
              lowerBlock.includes('fri') ||
              lowerBlock.includes('sat') ||
              lowerBlock.includes('sun') ||
              block.match(/\d{1,2}:\d{2}/) ||
              block.match(/\d{1,2}\s*(?:AM|PM|am|pm)/)) {
            allHoursBlocks.push(block);
          }
        }
      });

      // Extract business/company name from structured data or meta tags
      const names = [];
      
      // Try og:site_name meta tag
      const ogSiteName = document.querySelector('meta[property="og:site_name"]');
      if (ogSiteName?.content?.trim()) {
        names.push({ value: ogSiteName.content.trim(), source: 'og:site_name', confidence: 'high', type: 'business' });
      }
      
      // Try application-name meta tag
      const appName = document.querySelector('meta[name="application-name"]');
      if (appName?.content?.trim() && !names.find(n => n.value === appName.content.trim())) {
        names.push({ value: appName.content.trim(), source: 'application-name', confidence: 'high', type: 'business' });
      }
        ...
        try {
          ...
          for (const item of items) {
            ...
            if ((item['@type'] === 'Organization' || item['@type'] === 'LocalBusiness' || 
                 item['@type'] === 'Corporation' || item['@type'] === 'Store') && item.name) {
              const name = typeof item.name === 'string' ? item.name : item.name?.['@value'] || '';
              if (name && !names.find(n => n.value === name)) {
                names.push({ value: name, source: 'schema.org', confidence: 'high', type: 'business' });
              }
            }
            // Contact person name
            if (item['@type'] === 'Person' && item.name) {
              const name = typeof item.name === 'string' ? item.name : item.name?.['@value'] || '';
              if (name && !names.find(n => n.value === name)) {
                names.push({ value: name, source: 'schema.org', confidence: 'medium', type: 'person' });
              }
            }
          }
        } catch {}
      ...
      // Try title tag as business name if we don't have one yet
      const businessNames = names.filter(n => n.type === 'business');
      if (businessNames.length === 0 && document.title) {
        // Split on common title separators and take the first part
        // Using Unicode for em dash (—) and en dash (–) to avoid regex issues
        const title = document.title.split(/[|•:\u2014\u2013-]/)[0].trim();
        if (title && title.length > 2 && title.length < 100) {
          names.push({ value: title, source: 'title', confidence: 'medium', type: 'business' });
        }
      }
      ...
      const allHours = [...new Set([...hoursFromText, ...allHoursBlocks])].filter(hour => hour.length > 5);
      ...
      const phones = allPhones.map(phone => ({
        value: phone.replace(/\s+/g, ' ').trim(), // Clean spacing
      ...
      const hours = allHours.map(hour => ({
        value: hour.trim(),
        source: hoursFromText.includes(hour) ? 'text-pattern' : 'hours-element',
        confidence: hoursFromText.includes(hour) ? 'medium' : 'high'
      }));

      return { emails, phones, addresses, hours, names };
    } catch (error) {
...
/**
 * Extract only contact information (lightweight version)
 * Optimized for users who only want contact details without design tokens
 *
 * @param {string} url - Target URL to analyze
 * @param {Object} spinner - Ora spinner instance for progress updates
 * @param {Object} browser - Pre-configured browser instance
 * @param {Object} options - Configuration options
 * @returns {Object} Contact information only
 */
export async function extractContactsOnly(
  url,
  spinner,
  browser,
  options = {}
  ...
  try {
    ...
    const finalUrl = page.url();
    spinner.text = "Extracting contact information...";
    ...
    if (!contacts || (!contacts.emails?.length && !contacts.phones?.length && !contacts.addresses?.length && !contacts.hours?.length && !contacts.names?.length)) {
      console.log(chalk.hex('#FFB86C')(`  ⚠ No contact information found on this page`));
    }
