...
export async function extractBranding(
  ...
  try {
    ...
    console.log(contacts.emails?.length > 0 || contacts.phones?.length > 0 || contacts.addresses?.length > 0 || contacts.hours?.length > 0 || contacts.names?.length > 0 ? 
      chalk.hex('#50FA7B')(`  ✓ Contacts: ${contacts.emails?.length || 0} emails, ${contacts.phones?.length || 0} phones, ${contacts.addresses?.length || 0} addresses, ${contacts.hours?.length || 0} hours, ${contacts.names?.length || 0} names`) : 
      chalk.hex('#FFB86C')(`  ⚠ Contacts: 0 found`));
    console.log();
...
async function extractContacts(page) {
  return await page.evaluate(() => {
    try {
      ...
      const addressRegex = /(?:\d{1,5}\s+(?:[A-Z0-9\s]+(?:St|Rd|Ave|Blvd|Dr|Ln|Ct|Way|Pl|St\.|Rd\.|Ave\.|Blvd\.|Dr\.|Ln\.|Ct\.|Way\.|Pl\.)?)\s+(?:[A-Z0-9\s,]+(?:Apt|Unit|Suite|#)\s*\d*)?(?:,\s*([A-Z]{2}))?\s+(\d{5}(?:-\d{4})?))|([A-Z0-9\s,]+,\s*[A-Z]{2}\s+\d{5}(?:-\d{4})?)/gi;
