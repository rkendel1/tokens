...
/**
 * Merge contact information from multiple pages.
 * Deduplicates by value and keeps highest confidence.
 * Note: Uses shallow copy with spread operator - nested objects/arrays will be referenced.
 */
function mergeContacts(results) {
  // Confidence priority for deduplication
  const confidencePriority = { high: 3, medium: 2, low: 1 };

  const mergeContactArray = (arrays) => {
    const map = new Map();
    arrays.forEach(arr => {
      (arr || []).forEach(item => {
        ...
        if (!map.has(key)) {
          ...
          const existingScore = confidencePriority[existing.confidence] || 0;
          const newScore = confidencePriority[item.confidence] || 0;
  ...
  return {
    emails: mergeContactArray(results.map(r => r.contacts?.emails)),
    phones: mergeContactArray(results.map(r => r.contacts?.phones)),
    addresses: mergeContactArray(results.map(r => r.contacts?.addresses)),
    hours: mergeContactArray(results.map(r => r.contacts?.hours)),
    names: mergeContactArray(results.map(r => r.contacts?.names)),
  };
}
...
export function mergeResults(results) {
  ...
  return {
    ...
    contacts: mergeContacts(results),
